...
void	update_edge_flows(
	t_map *prev,
	int64_t augmenting_flow,
	t_graph_node *s,
	t_graph_node *t)
{
	t_graph_node	*node;
	...
	t_graph_node *t)
{
	...
	{
		prev = map_new();
		augmenting_flow = find_augmenting_flow(&prev, s, t);
		if (augmenting_flow <= 0)
			break ;
		update_edge_flows(&prev, augmenting_flow, s, t);
